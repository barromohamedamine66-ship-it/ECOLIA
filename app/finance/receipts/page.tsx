  ...
  if (loading || !receipt) {
    return (
      <div className="min-h-screen bg-slate-100 flex items-center justify-center p-6 text-slate-500">
        <p className="font-bold">Chargement du reçu de paiement officiel...</p>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-slate-100 dark:bg-slate-900 text-slate-800 p-4 md:p-8 flex flex-col items-center">
      ...
      <div className="max-w-2xl w-full mb-6 flex items-center justify-between print:hidden">
        <Link href="/finance" className="px-4 py-2 bg-white dark:bg-slate-800 border rounded-xl text-xs font-bold shadow-sm hover:bg-slate-50">
          ← Retour aux Finances
        </Link>
        <button
          onClick={handlePrint}
      ...
      <div className="max-w-2xl w-full bg-white text-slate-900 p-8 rounded-3xl shadow-2xl border border-slate-200 print:shadow-none print:border-none print:p-0">
        ...
        <div className="flex justify-between items-start pb-6 border-b-2 border-slate-900">
          <div>
            <span className="text-xl font-black text-emerald-900 block">{receipt.school.name}</span>
            <span className="text-xs italic text-amber-700 font-medium block">« {receipt.school.motto} »</span>
            <span className="text-[11px] text-slate-500 block">{receipt.school.address} • Tél: {receipt.school.phone}</span>
          </div>
        ...
        <div className="grid grid-cols-2 gap-6 pt-6 border-t text-xs">
          ...
          <div className="text-right flex flex-col items-end">
            ...
            <div className="w-20 h-20 rounded-full border-2 border-dashed border-emerald-700/60 flex items-center justify-center text-[9px] font-black text-emerald-800 uppercase text-center rotate-[-10deg]">
              GROUPE HORIZON<br/>CAISSE
            </div>
          </div>
        </div>
      </div>
